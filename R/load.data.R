...
load.data <- function(record.codes, token){
  ...
  for(i in 1:length(record.codes)){
    ...
    if( !is.character(record.codes[i]) | !is.character(token) ){
      ...
      # get.package.id <- fromJSON(paste("https://api.boldsystems.org:6543/dropin/datastore_load_data?recordset_codes=",record.codes[i],"&t=",token, sep=""))
      ...
      get.package.id <- fromJSON(paste("https://api.boldsystems.org/dropin/datastore_load_data?recordset_codes=",record.codes[i],"&t=",token, sep=""))
